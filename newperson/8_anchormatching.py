...
class AnchorHead(nn.Module):
    ...
    def get_pos_anchors(self, fpn_feats, gt_bboxes, gt_labels, img_metas, cfg, gt_bboxes_ignore=None):
        ...
        assert len(gt_labels) == len(np.unique(pos_assigned_gt_inds))

        match_num_list = []
        for gt_label in gt_labels:
            gt_matches = pos_assigned_gt_inds[pos_assigned_gt_inds==gt_label]
            match_num_list.append(len(gt_matches))

        return anchor_pos, labels_pos, match_num_list
...
def bbox_draw(img, bboxes, labels, is_gt=True,gt_segms=[]):
	width = img.shape[1]
	height = img.shape[0]
	...
	for i,bbox in enumerate(bboxes):
		...
		textcolor = (0,0,0) if is_gt else allcolors[int(labels[i]) % 6]
		textinfo = '{}'.format(int(labels[i]))
...
if __name__ == '__main__':
	...
	config_file = './retinanet_r50_fpn_1x_r2.py'
	...
	images_save_path = './val2017_vis'
	if not os.path.exists(images_save_path):
		os.mkdir(images_save_path)

	dataset_train = build_dataset(model.cfg.data.train)
	match_num_lists = []
	for idx in range(20):
		img_path, fpn_feats, gt_bboxes, gt_labels, img_metas, gt_segms = get_img_meta(model, idx, dataset_train)
		anchor_pos, labels_pos, match_num_list = anchor_head.get_pos_anchors(fpn_feats, gt_bboxes, gt_labels, img_metas, model.cfg)
		match_num_lists.extend(match_num_list)
# img = cv2.imread(img_path)
		# draw_gt_img = bbox_draw(img, gt_bboxes, gt_labels, True, gt_segms)
		# draw_anchor_img = bbox_draw(draw_gt_img, anchor_pos, labels_pos, False)

		# cv2.imwrite('{}/{}'.format(images_save_path, img_path.split('/')[-1]),draw_anchor_img)
	np.save("match_num_lists.npy", match_num_lists) #show_histgram(match_num_lists)